...
if hasattr(model, 'np_adapter') and model.np_adapter is not None:
    adapter_params = sum(p.numel() for p in model.np_adapter.parameters())
    adapter_trainable = all(p.requires_grad for p in model.np_adapter.parameters())
    print(f"NP adapter params: {adapter_params:,} (trainable: {adapter_trainable})")
...
for epoch in range(last_epoch, config.EPOCHS):
    ...
    if (epoch + 1) % 10 == 0 or epoch > 45:
      ...
      print(f"Epoch {epoch+1} - Val Loss: {val_loss:.4f}, Val F1 best: {val_f1_best:.4f}, "
            f"Val F1 fixed: {val_f1_fixed:.4f}, Val F1 mod: {val_f1_mod:.4f}")
